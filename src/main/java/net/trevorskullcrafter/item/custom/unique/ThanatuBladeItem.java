...
public class ThanatuBladeItem extends SwordItem implements StyleUtil.StyleSwitchable, PillarAligned {
    public ThanatuBladeItem(ToolMaterial toolMaterial, Settings settings) { super(toolMaterial, settings.component(ModDataComponentTypes.MAX_STYLE, 2)); }

    @Override public Formatting getStyleSwitchFormatting(ItemStack stack) {
        return (TSItems.getOrSetComponent(stack, ModDataComponentTypes.STYLE, 1) % 2) == 0? Formatting.DARK_PURPLE : Formatting.LIGHT_PURPLE;
    }
    ...
    @Override public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if(TSItems.getOrSetComponent(stack, ModDataComponentTypes.STYLE, 1) == 1){
			GlobalPos pos = stack.get(ModDataComponentTypes.STORED_POSITION);
